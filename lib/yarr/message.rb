...
module Yarr
  class Message
    ...
    def reply_to(message)
      ...
      p command: command, target: target
      begin
        ast = @ri_parser.parse(target) unless target.empty?
        p ast.inspect
        response = send("handle_#{command}", ast)
        response << ' , ' << ast[:stuff] if ast.key?(:stuff)
        response
      rescue Parslet::ParseFailed
        "I did not understand subject #{target}."
      rescue NoMethodError => e
        puts e
        puts e.backtrace
        "I did not understand command #{command}."
      end
    end
