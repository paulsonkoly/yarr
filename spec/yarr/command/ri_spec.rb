    ...
    RSpec.describe Ri do
      ...
      describe RiClassName do
        describe '#handle' do
          ...
          context 'with a module name from a non-core origin' do
            before do
              allow(Query::Klass).to receive(:where).and_return(dataset)
            end

            let(:ast) { Yarr::AST.new(class_name: 'Abbrev') }
            let(:dataset) { class_double(Query::Klass, all: [klasses[1]]) }
          ...
          context 'when there is a hit from core' do
            ...
            let(:dataset) { class_double(Query::Klass, all: klasses) }
          ...
          context 'when there are multiple choices but origin name is the lower case class name' do
            ...
            let(:dataset) { class_double(Query::Klass, all: [klasses[2]]) }
      ...
      describe RiMethodName do
        describe '#handle' do
          ...
          before do
            allow(Query::Method).to receive(:where).and_return(methods[0..0])
          end
