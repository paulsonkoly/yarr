...
module Yarr
  RSpec.describe CommandDispatcher do
    ...
    it 'handles "ast aa"' do
      handler = dispatcher.dispatch(command: 'ast', method_name: 'aa')
    ...
    context 'with ri command' do
      it 'handles method names' do
        handler = dispatcher.dispatch(command: 'ri', method_name: 'aa')

        expect(handler).to be_a Command::RiMethodName
      end

      it 'handles class names' do
        handler = dispatcher.dispatch(command: 'ri', class_name: 'Aa')

        expect(handler).to be_a Command::RiClassName
      end

      it 'handles instance methods' do
        handler = dispatcher.dispatch(command: 'ri', instance_method: 'Aa')

        expect(handler).to be_a Command::RiInstanceMethod
      end

      it 'handles class methods' do
        handler = dispatcher.dispatch(command: 'ri', class_method: 'Aa')
    ...
    context 'with list command' do
      it 'handles method names' do
        handler = dispatcher.dispatch(command: 'list', method_name: 'aa')

        expect(handler).to be_a Command::ListMethodName
      end

      it 'handles class names' do
        handler = dispatcher.dispatch(command: 'list', class_name: 'Aa')

        expect(handler).to be_a Command::ListClassName
      end

      it 'handles instance methods' do
        handler = dispatcher.dispatch(command: 'list', instance_method: 'Aa')

        expect(handler).to be_a Command::ListInstanceMethod
      end

      it 'handles class methods' do
        handler = dispatcher.dispatch(command: 'list', class_method: 'Aa')
